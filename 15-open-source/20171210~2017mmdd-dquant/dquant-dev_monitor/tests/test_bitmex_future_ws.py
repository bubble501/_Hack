...
# 继承自unittest.TestCase，表明其是一个单元测试子类
class BitmexFutureTestSuite(unittest.TestCase):
    ...
    '''
    unittest.skip()装饰器：
    当运行用例时，有些用例可能不想执行，可用该装饰器屏蔽该条测试用例
    一种常见的用法就是比如说向调试某一个测试用例，可以先屏蔽其他用例

    unittest.skip(reason)中的reason就是屏蔽的原因内容
    '''
