>[Python量化交易平台开发教程系列4-事件驱动引擎原理和使用](http://www.vnpy.org/basic-tutorial-4.html)
...
1. 每个5分钟，通过新浪财经网页的公开API读取一次沪深300成分股的价格，根据当日涨幅进行排序后输出到电脑屏幕上
2. 每个1秒钟，检查一次最新收到的股指期货TICK数据，更新K线和其他技术指标，检查是否满足趋势策略的下单条件，若满足则执行下单
