...
接下来看一下哈希算法在比特币系统中的作用！假设我们相信一个安全的哈希算法，如果H(x)=H(y)，则心=y

如果输入的内容就是文件的内容，而两个文件的Hash相同，说明文件没有被修改过

![image](./image/12.png)

和文件类似，如果两份数据的哈希相同，说明两份数据相同

![image](./image/13.png)

比特币是用哈希算法来保证所有的交易不可修改（计算和记录交易的哈希），如果交易被篡改，那么哈希验证将无法通过，说明这个区块是无效的

比特币使用了两种哈希算法：

* 一种是两次SHA-256：hash256 = sha256(sha256(data))
* 另一种是先计算SHA-256，再计算RipeMD160：hash160 = ripemd160(sha256(data))

### 区块链如何使用哈希算法防止交易被篡改？

区块本身记录的主要数据是一系列交易，所以区块链首先要保证任何交易数据都不可修改

区块的头部有一个Merkle Hash的字段，它记录了本区块所有交易的Hash

![image](./image/14.png)

Merkle Hash是把一系列数据的Hash根据一个简单算法列成一个汇总的Hash（假设一个区块有3个交易，我们对每个交易数据做dhash，得到3个Hash值（因为存在单数，所以将最后一个数字复制一份得到双数继续下一层运算），再逐层Hash，得到最终的Merkle Hash值）

![image](./image/15.png)

区块本身使用Block Hash作为区块唯一标识，一个区块的Hash并没有记录在区块的头部，而是通过计算区块头部的Hash得到的

区块头部的Prev Hash记录了上一个区块的Hash，这样就可以通过Prev Hash追踪到上一个区块，由于下一个区块的Prev Hash又会指向当前区块，所以这些区块串起来就变成了区块链

如果一个恶意攻击者修改了某个区块链中的某个交易，那么这个区块的Merkle Hash验证就不会通过，所以攻击者只能重新计算Merkle Hash，然后把区块头的Merkle Hash也修改了，这个时候引起连锁反应，也就是这个区块本身的Block Hash也就变了，所以下一个区块指向它的链接就断掉了

![image](./image/16.png)

攻击者为了继续攻击，就必须计算这个区块的Hash，并且将后续所有区块的Hash重新计算并且伪造出来，才能够修改整个区块链。所以修改一个区块的成本就很高了，为了修改后续所有区块，这个攻击者必须拥有全网52%的算力

![image](./image/17.png)

而且由于正常的区块链在不断增长，同一个区块的修改难度会随着时间的推移而不断增加
